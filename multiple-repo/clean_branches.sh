...
run_branch_cleanup() {
    ...
    local proceed_with_deletion=false
...
while true; do
    ...
    engines_raw=$(find . -maxdepth 1 -mindepth 1 -type d -not -path './.*' | sed 's|^\./||' | sort)
    if [ -z "$engines_raw" ]; then
      echo "Error: No engines (subdirectories) found."
      exit 1
    fi
    ...
    echo "Please select the engine to process:"
    PS3="Your choice (type 'bye' to exit): "
    
    select opt in "${engines[@]}"; do
      if [[ "$REPLY" == "bye" ]]; then
          echo "Goodbye!"
          exit 0
      fi
      ...
      if [[ -z "$opt" ]]; then
        echo "Invalid selection: '$REPLY'. Please select a number from the list or type 'bye'."
        continue
      fi

      (
        cd "$opt" && run_branch_cleanup "$opt"
      )
      break
    done
done